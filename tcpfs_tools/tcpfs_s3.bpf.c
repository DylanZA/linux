...
#define ctx tctx
	...
	S3_LOG("tcpfs_s3: build_request id=%llu op=%u path_len=%u",
	...
	if (ctx->op == TCPFS_OP_READ) {
		...
		if (ret)
			return ret;
		S3_LOG("tcpfs_s3: built read id=%llu payload_len=%u",
	...
	if (ctx->op == TCPFS_OP_READDIR) {
		...
		if (ret)
			return ret;
		S3_LOG("tcpfs_s3: built readdir id=%llu payload_len=%u",
		       ctx->id, ctx->payload_len);
		return 0;
	}
	...
	if (ret)
		return ret;
	S3_LOG("tcpfs_s3: built attr id=%llu payload_len=%u",
	       ctx->id, ctx->payload_len);
	return 0;
#undef ctx
}
...
{
#define ctx tctx
	S3_LOG("tcpfs_s3: frame_tx id=%llu payload_len=%u",
	       ctx->id, ctx->payload_len);
...
#define ctx tctx
	...
	body = bpf_tcpfs_rx_find_str(ctx, "\r\n\r\n");
	if (body < 0)
		return -TCPFS_EAGAIN;
	body += 4;
	ctx->frame_len = body;
	if (ctx->op == TCPFS_OP_READDIR) {
		...
		if (ret)
			return -TCPFS_EPROTO;
		if (body_len > TCPFS_PAYLOAD_MAX ||
		    (__u32)body > TCPFS_PAYLOAD_MAX - (__u32)body_len)
			return -TCPFS_EMSGSIZE;
		if (ctx->rx_len < body + body_len)
			return -TCPFS_EAGAIN;
		ctx->frame_len = body + body_len;
	}
	S3_LOG("tcpfs_s3: unframe_rx id=%llu rx_len=%u",
	       ctx->id, ctx->rx_len);
	return 0;
#undef ctx
}
...
#define ctx tctx
	...
	S3_LOG("tcpfs_s3: handle_response id=%llu op=%u rx_len=%u",
	...
	if (ctx->op == TCPFS_OP_READ) {
		...
		ctx->result.flags = TCPFS_RESULT_F_STREAM |
					   TCPFS_RESULT_F_SIZE_VALID;
		ctx->result.offset = ctx->offset;
		ctx->result.size = body_len;
		ctx->result.rx_offset = body;
		ctx->result.payload_len = n;
		S3_LOG("tcpfs_s3: result read id=%llu rx_offset=%u payload_len=%u size=%llu",
		       ctx->id, ctx->result.rx_offset,
	...
	if (ctx->op == TCPFS_OP_READDIR) {
		...
		if (ret)
			return ret;
		S3_LOG("tcpfs_s3: result readdir id=%llu payload_len=%u",
	...
	S3_LOG("tcpfs_s3: result attr id=%llu size=%llu",
...
{
#define ctx tctx
	S3_LOG("tcpfs_s3: unsolicited id=%llu result_id=%llu",
