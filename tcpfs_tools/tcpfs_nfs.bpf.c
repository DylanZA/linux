...
/*
 * Read-only NFSv4.0 protocol implementation for tcpfs.
 *
 * Each request starts at the NFSv4 pseudo-filesystem root and resolves the
 * mount argument followed by the tcpfs path in one COMPOUND.  READ uses the
...
int BPF_PROG(tcpfs_nfs_build_request, struct tcpfs_ctx *tctx)
{
	...
	NFS_LOG("tcpfs_nfs: build id=%llu op=%u path_len=%u",
	...
	NFS_LOG("tcpfs_nfs: built id=%llu nops=%u bytes=%u",
		tctx->id, nops, tctx->payload_len);
	return 0;
}

SEC("struct_ops/frame_tx")
int BPF_PROG(tcpfs_nfs_frame_tx, struct tcpfs_ctx *tctx)
{
	struct nfs_tx tx = {
		.ctx = tctx,
	};
...
int BPF_PROG(tcpfs_nfs_unframe_rx, struct tcpfs_ctx *tctx)
{
	__u32 total;
	int ret;
...
int BPF_PROG(tcpfs_nfs_handle_response, struct tcpfs_ctx *tctx)
{
	...
	if (ret == -TCPFS_EAGAIN && total <= tctx->rx_len)
		ret = -TCPFS_EPROTO;
	NFS_LOG("tcpfs_nfs: reply id=%llu op=%u type=%u err=%d bytes=%u",
...
int BPF_PROG(tcpfs_nfs_on_unsolicited, struct tcpfs_ctx *tctx)
{
	return 0;
}

SEC(".struct_ops.link")
struct tcpfs_ops tcpfs_nfs = {
	.name = "tcpfs_nfs",
	.conn_style = TCPFS_CONN_NEW,
	.build_request = (void *)tcpfs_nfs_build_request,
	.frame_tx = (void *)tcpfs_nfs_frame_tx,
	.unframe_rx = (void *)tcpfs_nfs_unframe_rx,
	.handle_response = (void *)tcpfs_nfs_handle_response,
	.on_unsolicited = (void *)tcpfs_nfs_on_unsolicited,
};
