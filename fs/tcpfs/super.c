...
static int tcpfs_fill_super(struct super_block *sb, struct fs_context *fc)
{
	...
	pr_info("tcpfs: mount begin ops=%s servers=%u arg=%s timeout_ms=%u buf_size=%u buf_count=%u\n",
		sbi->opts.ops_name, sbi->nr_servers, sbi->opts.arg ?: "",
	...
	pr_info("tcpfs: mount ready ops=%s arg=%s\n",
		sbi->opts.ops_name, sbi->opts.arg ?: "");
...
static void tcpfs_kill_sb(struct super_block *sb)
{
	...
	kill_anon_super(sb);
	if (sbi) {
		pr_info("tcpfs: unmount ops=%s arg=%s\n",
			sbi->opts.ops_name ?: "", sbi->opts.arg ?: "");
		tcpfs_pool_destroy(sbi);
		tcpfs_bpf_put(sbi->bpf_ops);
		tcpfs_free_opts(&sbi->opts);
