...
/* Keep the complete maximum ordinary TCP header CPU-readable. */
#define VIRTNET_MP_PREFIX_LEN	(sizeof(struct virtio_net_hdr_mrg_rxbuf) + \
				 ETH_HLEN + VLAN_HLEN + sizeof(struct ipv6hdr) + \
...
static int virtnet_create_page_pools(struct virtnet_info *vi)
{
	...
	netdev_assert_locked_ops_compat_or_invisible(vi->dev);
	...
	for (i = 0; i < vi->max_queue_pairs; i++) {
		...
		allow_unreadable =
			!!__netif_get_rx_queue(vi->dev, i)->mp_params.mp_ops;

		err = virtnet_create_page_pool(vi, i, allow_unreadable,
					       &rq->page_pool,
...
static int virtnet_reset_destroy_page_pools(struct virtnet_info *vi)
{
	int i, err, ret = 0;
	...
	if (!ret) {
		...
		vi->page_pools_need_reset = false;
	}
				   ...
				   void *per_queue_mem, int index)
{
	...
	allow_unreadable =
		!!__netif_get_rx_queue(dev, index)->mp_params.mp_ops;

	return virtnet_create_page_pool(vi, index, allow_unreadable,
					&mem->page_pool,
...
{
	struct virtnet_info *vi = netdev_priv(dev);
	bool reset_page_pools = vi->page_pools_need_reset;
	int i, err, reset_err;

	for (i = 0; !reset_page_pools && i < vi->max_queue_pairs; i++) {
		struct receive_queue *rq = &vi->rq[i];
		bool has_provider;

		has_provider =
			!!__netif_get_rx_queue(dev, i)->mp_params.mp_ops;
		if (rq->page_pool &&
		    page_pool_is_unreadable(rq->page_pool) != has_provider)
			reset_page_pools = true;
	}

	if (reset_page_pools) {
		vi->page_pools_need_reset = true;
		err = virtnet_reset_destroy_page_pools(vi);
		if (err)
			return err;
	}
...
err_enable_qp:
	...
	reset_err = virtnet_reset_destroy_page_pools(vi);
	if (reset_err) {
		vi->page_pools_need_reset = true;
		netdev_warn(dev, "failed to reset receive queues: %d\n",
			    reset_err);
	}
...
static void virtnet_freeze_prepare(struct virtio_device *vdev)
{
...
static void virtnet_free_queues(struct virtnet_info *vi)
{
	...
	for (i = 0; i < vi->max_queue_pairs; i++) {
		__netif_napi_del_locked(&vi->rq[i].napi);
		__netif_napi_del_locked(&vi->sq[i].napi);
	}

	/* We called __netif_napi_del_locked(),
...
static int virtnet_alloc_queues(struct virtnet_info *vi)
{
	...
	for (i = 0; i < vi->max_queue_pairs; i++) {
		vi->rq[i].pages = NULL;
		netif_napi_add_config_locked(vi->dev, &vi->rq[i].napi,
					     virtnet_poll, i);
		vi->rq[i].napi.weight = napi_weight;
		set_bit(NAPI_STATE_NO_BUSY_POLL, &vi->sq[i].napi.state);
		netif_napi_add_weight_locked(vi->dev, &vi->sq[i].napi,
					     virtnet_poll_tx,
...
static void virtnet_freeze_vqs(struct virtio_device *vdev)
{
	struct virtnet_info *vi = vdev->priv;

	virtnet_freeze_prepare(vdev);
