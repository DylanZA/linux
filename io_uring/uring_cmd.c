...
/*
 * Locate a cancelable uring command matching @cd and request its cancellation.
 * Used by io_try_cancel() so filesystem io_uring commands share the ordinary
 * cancellation path instead of lingering until completion.
 *
 * Return: 0 when a matching command was cancelled, -ENOENT when none matched.
 */
